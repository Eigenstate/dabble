...
#
# $Id: chirality.tcl,v 1.24 2013/04/15 15:38:41 johns Exp $
#
  ...
  for {set i 0} {$i < [llength $chiralCenters]} {incr i} {
    ...
    if {[$sel num] > 0} {
      ...
      foreach atomnames $atomnamesList {
        ...
        set selatom1 [atomselect $molid "($seltext) and resname $resnamesList and name [lindex $atomnames 2] $altlocSelector"]
        set selatom2 [atomselect $molid "($seltext) and resname $resnamesList and name [lindex $atomnames 3] $altlocSelector"]
        set selatom3 [atomselect $molid "($seltext) and resname $resnamesList and name [lindex $atomnames 4] $altlocSelector"]
        # Does this structure have hydrogen atoms?
        if {[$selatomH num] > 0} {
          set hasH 1

          # Deal with chiral center(s) involving inconsistent naming of 
          # hydrogen atom between PDB, CHARMM, and Amber. We use a list of
          # possible atom names that should be chosen in order of preference.
          if { [llength [lindex $atomnames 0]] > 1 } {
            set foundNames [lsort -unique [$selatomH get name]]
            foreach name [lindex $atomnames 0] {
              if { [lsearch -exact $foundNames $name] != -1 } {

                # Workaround for CHARMM naming of O2'-associated hydrogen. This workaround became necessary
                # when the order of preference for hydrogen naming was updated in $chiralCenters to support
                # PDB files containing DNA/RNA hybrids.
                # 
                # If the picked hydrogen name was H2', make sure it is not bound to O2'. Otherwise, pick the
                # next possible hydrogen name.
                if {$name == "H2'" && [lindex $atomnames 4] == "O2'"} {
                  set selTest [atomselect $molid "($seltext) and resname $resnamesList and name $name $altlocSelector"]
                  set idTest [lindex [$selTest get index] 0]
                  $selTest delete
                  set selTest [atomselect $molid "index $idTest"]
                  set idBondedTo [$selTest getbonds]
                  $selTest delete
                  set selTest [atomselect $molid "index $idBondedTo"]
                  set nameBondedTo [$selTest get name]
                  $selTest delete
                  if { [lsearch -exact $nameBondedTo "O2'"] != -1 } {
                    continue
                  }
                }

                set useName $name
                lset atomnames 0 $useName
                break
              }
            }
            $selatomH delete
            set selatomH [atomselect $molid "($seltext) and resname $resnamesList and name $useName $altlocSelector"]
          }
