#
# $Id: fftk_guiProcs.tcl,v 1.32 2014/12/04 21:01:57 mayne Exp $
#
...
proc ::ForceFieldToolKit::gui::bparAnalyzeMissingPars {} {
    ...
    set RefParList $::ForceFieldToolKit::BuildPar::RefParList
    
    # run a sanity check
    if { ![::ForceFieldToolKit::BuildPar::sanityCheck analyzeMissingPars ] } { return }
    ...
    set molecPars [::ForceFieldToolKit::BuildPar::getMolecPars]
    ...
    update idletasks
    set bparAnalyzeMolID [molinfo top]
    ...
    # reprocess for uniqueness
    #foreach key [array names typeArray] {
    #    set typeArray($key) [lsort -integer -increasing -unique $typeArray($key)]
    #}
...
proc ::ForceFieldToolKit::gui::bparShowMissingParsElements {} {
    ...
    if { [lsearch [molinfo list] $bparAnalyzeMolID] == -1 } { return }

    # clear any existing objects
    ::ForceFieldToolKit::SharedFcns::ParView::clearParViewObjList -molid $bparAnalyzeMolID

    # set material
    ::ForceFieldToolKit::SharedFcns::ParView::addMaterialObj -molid $bparAnalyzeMolID -material Diffuse

    # build bonds
    ::ForceFieldToolKit::SharedFcns::ParView::addColorObj -molid $bparAnalyzeMolID -color red
    foreach item [.fftk_gui.hlf.nb.buildpar.missingPars.vizFrame.bondsTv selection] {
        foreach indSet [.fftk_gui.hlf.nb.buildpar.missingPars.vizFrame.bondsTv set $item indsList] {
            ::ForceFieldToolKit::SharedFcns::ParView::addParObject -molid $bparAnalyzeMolID -type bond -indices $indSet
        }
    }

    # build angles
    ::ForceFieldToolKit::SharedFcns::ParView::addColorObj -molid $bparAnalyzeMolID -color blue
    foreach item [.fftk_gui.hlf.nb.buildpar.missingPars.vizFrame.anglesTv selection] {
        foreach indSet [.fftk_gui.hlf.nb.buildpar.missingPars.vizFrame.anglesTv set $item indsList] {
            ::ForceFieldToolKit::SharedFcns::ParView::addParObject -molid $bparAnalyzeMolID -type angle -indices $indSet
        }
    }

    # build dihedrals
    ::ForceFieldToolKit::SharedFcns::ParView::addColorObj -molid $bparAnalyzeMolID -color green
    foreach item [.fftk_gui.hlf.nb.buildpar.missingPars.vizFrame.dihedralsTv selection] {
        foreach indSet [.fftk_gui.hlf.nb.buildpar.missingPars.vizFrame.dihedralsTv set $item indsList] {
            ::ForceFieldToolKit::SharedFcns::ParView::addParObject -molid $bparAnalyzeMolID -type dihedral -indices $indSet
        }
    }

    # build nonbonded
    ::ForceFieldToolKit::SharedFcns::ParView::addColorObj -molid $bparAnalyzeMolID -color magenta
    foreach item [.fftk_gui.hlf.nb.buildpar.missingPars.vizFrame.nonbondedTv selection] {
        foreach indSet [.fftk_gui.hlf.nb.buildpar.missingPars.vizFrame.nonbondedTv set $item indsList] {
            ::ForceFieldToolKit::SharedFcns::ParView::addParObject -molid $bparAnalyzeMolID -type atom -indices $indSet
        }
    }    
}
#======================================================
proc ::ForceFieldToolKit::gui::bparShowMissingParsRepsOLD {parType} {
    # shows a representation of the selected tv entries in VMD
    # via a CPK representation (design based on dihedral viz)

    # input: parType = bonds|angles|dihedrals|nonbonded

    # localize relevant variables
    variable bparAnalyzeMolID
    variable bparAnalyzeBondRepID
    variable bparAnalyzeAngleRepID
    variable bparAnalyzeDihRepID
    variable bparAnalyzeNonbRepID

    # make sure that the molecule loaded during the analyze proc still exists
    if { [lsearch [molinfo list] $bparAnalyzeMolID] == -1 } {
        foreach var {bparAnalyzeMolID bparAnalyzeBondRepID bparAnalyzeAngleRepID bparAnalyzeDihRepID bparAnalyzeNonbRepID} { set ele -1 }
        return
    }
    ...
    set showIndList {}
    foreach item [.fftk_gui.hlf.nb.buildpar.missingPars.vizFrame.${parType}Tv selection] {
        foreach ind [.fftk_gui.hlf.nb.buildpar.missingPars.vizFrame.${parType}Tv set $item indsList] {
            lappend showIndList $ind
        }
    }

    # when showIndList == 0, escape has been pushed -> turn off rep
    if { [llength $showIndList] == 0 } {
        switch $parType {
            {bonds}     {mol showrep $bparAnalyzeMolID $bparAnalyzeBondRepID  off}
            {angles}    {mol showrep $bparAnalyzeMolID $bparAnalyzeAngleRepID off}
            {dihedrals} {mol showrep $bparAnalyzeMolID $bparAnalyzeDihRepID   off}
            {nonbonded} {mol showrep $bparAnalyzeMolID $bparAnalyzeNonbRepID  off}
        }
        return
    }

    # change the relevant rep selection based on the tv selection data
    switch $parType {
        {bonds} {
            if { [llength $showIndList] == 0 } {
                mol showrep $bparAnalyzeMolID $bparAnalyzeBondRepID off
                return
            }

            if { $bparAnalyzeBondRepID != -1 } {
                mol modselect $bparAnalyzeBondRepID $bparAnalyzeMolID "index $showIndList"
            } else {
                mol selection "index $showIndList"
                mol representation CPK
                mol color {ColorID 7}
                mol addrep $bparAnalyzeMolID
                set bparAnalyzeBondRepID [expr {[molinfo top get numreps] - 1}]
            }
            mol showrep $bparAnalyzeMolID $bparAnalyzeBondRepID on
        }

        {angles} {
            if { $bparAnalyzeAngleRepID != -1 } {
                mol modselect $bparAnalyzeAngleRepID $bparAnalyzeMolID "index $showIndList"
            } else {
                mol selection "index $showIndList"
                mol representation CPK
                mol color {ColorID 1}
                mol addrep $bparAnalyzeMolID
                set bparAnalyzeAngleRepID [expr {[molinfo top get numreps] - 1}]
            }
            mol showrep $bparAnalyzeMolID $bparAnalyzeAngleRepID on
        }

        {dihedrals} {
            if { $bparAnalyzeDihRepID != -1 } {
                mol modselect $bparAnalyzeDihRepID $bparAnalyzeMolID "index $showIndList"
            } else {
                mol selection "index $showIndList"
                mol representation CPK
                mol color {ColorID 0}
                mol addrep $bparAnalyzeMolID
                set bparAnalyzeDihRepID [expr {[molinfo top get numreps] - 1}]
            }
            mol showrep $bparAnalyzeMolID $bparAnalyzeDihRepID on
        }

        {nonbonded} {
            if { $bparAnalyzeNonbRepID != -1 } {
                mol modselect $bparAnalyzeNonbRepID $bparAnalyzeMolID "index $showIndList"
            } else {
                mol selection "index $showIndList"
                mol representation CPK
                mol color {ColorID 27}
                mol addrep $bparAnalyzeMolID
                set bparAnalyzeNonbRepID [expr {[molinfo top get numreps] - 1}]
            }
            mol showrep $bparAnalyzeMolID $bparAnalyzeNonbRepID on
        }
        default {
            return
        }
