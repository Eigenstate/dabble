...
#  grid $GridpdbInput -row 10 -column 1 -columnspan 2 -sticky nsew
 ...
 # $Notebook add $NBTab2 -text "Parameters" 
  ...
  $xMDFFDensityView configure -columns {file selection gscale mask maskres maskcutoff} -show {headings} -height 3
  $xMDFFDensityView heading file -text "Density Filename"
...
proc MDFFGUI::gui::add_density {} {
  ...
  set ADSelectDX [ttk::button $wad.hlf.dx -text "Browse" -command {MDFFGUI::gui::get_density} ]
...
proc MDFFGUI::gui::add_density_xmdff {} {
  ...
  set xADSelectDX [ttk::button $xwad.hlf.dx -text "Browse" -command {MDFFGUI::gui::get_refs} ]
  ...
  grid $xADLoadDensity -row 6 -column 0 -sticky nsew -columnspan 3
}
...
proc MDFFGUI::gui::load_density_list {} {
  ...
  if {[llength $MDFFGUI::settings::RefsList] !=  [llength $MDFFGUI::settings::xMDFFGridPDBSelList] && \
     [llength $MDFFGUI::settings::RefsList] !=  [llength $MDFFGUI::settings::xMDFFGScaleList]} {
       ...
       foreach file $MDFFGUI::settings::RefsList gscale $MDFFGUI::settings::xMDFFGScaleList seltext $MDFFGUI::settings::xMDFFGridPDBSelList mask $MDFFGUI::settings::xMDFFMaskList maskres $MDFFGUI::settings::xMDFFMaskResList maskcutoff $MDFFGUI::settings::xMDFFMaskCutoffList { 
...
#    file rename -force "fixed.pdb" MDFFGUI::$MDFFGUI::settings::CurrentDir 
    ...
    $xmdff $refs $reffiles {*}$refsteps {*}$CrystOption $bfs $mask $masks $maskres $masksres \
    $maskcutoff $maskscutoff $imd {*}$imdport {*}$imdfreq $imdwait $imdignore \
  ...
  if {$MDFFGUI::settings::xMDFF} {
    ...
    if {[expr ($ts-$MDFFGUI::settings::Minsteps-100) % $MDFFGUI::settings::RefSteps] == 0 && $justChanged != $ts || $ts == 1 && $justChanged != $ts} {
      for {set id 0} {$id < [llength $xMDFFMapIDs]} {incr id} {
        ...
        if {[molinfo $newmap get name] == $DensityIDName } { set DensityID $newmap }
...
proc MDFFGUI::gui::load_density_xmdff {} {
 # variable MapID
  ...
  set id [$xMDFFDensityView insert {} end -values [list $MDFFGUI::settings::CurrentRefsPath $MDFFGUI::settings::GridPDBSelText $MDFFGUI::settings::GScale $MDFFGUI::settings::Mask $MDFFGUI::settings::MaskRes $MDFFGUI::settings::MaskCutoff]]
